...
function locationUpdate() {
    ...
    // now
    const pos = SunCalc.getMoonPosition(now, locator.latitude, locator.longitude);
    moonPos.push(Object.assign({
        moon: true,
        now: true,
        relative: 0,
        h: now.getHours(),
    ...
    for (let i = -12; i < 12; i++) {
        ...
        const pos = SunCalc.getMoonPosition(date, locator.latitude, locator.longitude);
        moonPos.push(Object.assign({
            moon: true,
            relative: i,
            h: date.getHours(),
    ...
    const sunPos = [];
    // now
    const pos2 = SunCalc.getPosition(now, locator.latitude, locator.longitude);
